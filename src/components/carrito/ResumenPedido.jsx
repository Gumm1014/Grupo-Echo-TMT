function LineaPedido({ nombre, cant }) {
  return (
    <div className="linea-pedido">
      <span className="linea-nombre">{nombre} x {cant}</span>
      <span className="linea-precio">A cotizar</span>
    </div>
  );
}
...
function SinPrecio() {
  return (
    <div className="sin-precio">
      <p className="sin-precio-titulo">¿Por qué no hay precio?</p>
      <p className="sin-precio-texto">
        Los costos de la madera varían según stock y calidad. Cada presupuesto se calcula
        de forma personalizada para garantizar el mejor precio.
      </p>
    </div>
  );
}

function BtnWhatsapp({ carrito }) {
  ...
  return (
    <button className="btn-whatsapp" onClick={() => window.open(url, "_blank")}>
...
export default function ResumenPedido({ items, carrito }) {
  return (
    <aside className="resumen">
      ...
      <div className="lista-lineas">
        {items.map((item, i) => (
          <LineaPedido key={i} nombre={item.nombre} cant={item.cant} />
        ))}
      </div>

      <div className="total">
        <span className="total-label">Total estimado</span>
        <span className="total-dash">—</span>
        <span className="total-valor">Por confirmar</span>
      </div>

      <SinPrecio />
      <BtnWhatsapp carrito={carrito} />
    </aside>
  );
}